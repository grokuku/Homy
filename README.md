...
- `GET /api/elements` — global element catalogue (CRUD: `POST`, `GET /:id`, `PATCH /:id`, `DELETE /:id`; `GET /:id/usage`). An element may carry a `report` config (`{ type, baseUrl, hasApiKey }` — the API key is **never** returned) and an optional `healthUrl` (custom http(s) health probe, not a secret — returned like `url`)
- `POST /api/health/check` — probe a batch of element health URLs `{ urls: ["https://…/health", …] }` → `{ checkedAt, results: [{ url, ok, status, checkedAt, error }] }`. **JWT required**, **http(s) only** (any other scheme or an embedded credential → 400), ≤ 20 URLs per request, 5 s timeout, ~30 s per-URL in-memory cache
...
- **Custom health URLs** (`element.healthUrl`) are probed **server-side only** (the browser never fetches them). The server applies, before any `fetch()`: an **http(s)-only** scheme check (rejecting `file:`, `ftp:`, `gopher:`, …), a refusal of URLs embedding credentials (`user:pass@`), a **short timeout** (5 s), a per-URL **in-memory cache** (~30 s) and a **bound** of ≤ 20 URLs per request. A URL is never written to the logs (a query token cannot leak); only a stable error code (`timeout`/`unreachable`/`http_<status>`) is reported.
